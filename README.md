...
ScriptableObject 없이 컴포넌트 자체에 경로 데이터를 저장하므로, 런타임에서 API를 통해 포인트를 자유롭게 추가·수정·삭제할 수 있습니다. 경로 프리셋(원/다각형/별), 스냅샷 모핑, 독립 타이밍 에이전트, Tiling 스프라이트 리본(PathRibbon)을 제공합니다.
...
```
https://github.com/zzamjak-cloud/PathFollower.git
```

특정 버전 설치:

```
https://github.com/zzamjak-cloud/PathFollower.git#v1.0.0
```

> **Private 레포 안내**: 이 저장소가 private인 동안 git URL 설치는 GitHub 인증이 설정된 환경에서만 동작합니다.
> (Git Credential Manager 로그인 또는 `https://<PAT>@github.com/zzamjak-cloud/PathFollower.git` 형태의 Personal Access Token 사용)

### 요구 사항

- **Unity**: 6000.0 이상
- **의존성**: com.unity.ugui 2.0.0 (자동 설치)

### 기존 프로젝트에서 마이그레이션

기존에 `Assets/Plugins/CAT/PathFollower/` 폴더로 사용하던 프로젝트는 **해당 폴더를 삭제한 뒤** 이 패키지를 설치하세요. 스크립트 GUID가 보존되어 있어 기존 씬/프리팹의 컴포넌트 참조가 그대로 유지됩니다.
...
│   └── Resources/
│       └── PathRibbonUnlit.shader   # Sprite 모드 폴백 셰이더 (CAT/PathFollower/Ribbon-Unlit)
├── Editor/
...
- 네임스페이스: `CAT.Utility`
- 어셈블리: `CAT.PathFollower` (Runtime) / `CAT.PathFollower.Editor` (Editor)
